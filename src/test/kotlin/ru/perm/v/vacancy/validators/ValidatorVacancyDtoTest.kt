...
class ValidatorVacancyDtoTest {
    ...
    fun checkValidateMessageFor_EmptyName() {
        ...
        assertEquals(
            "VacancyDto(n=1, name='', comment='comment', company=CompanyDto(n=100, name='testCompany'), contact=ContactDto(n=10, name=, email=, phone=, comment=)) содержит ошибки: размер должен находиться в диапазоне от 5 до 50\n",
            excpt.message
        )
    }
}